...
public class PersonRepositoryImp implements PersonRepository {
    ...
    public PersonRepositoryImp(MongoClient mongoClient){
    ...
    public List<Person> saveAll(List<Person> people) {
        try (ClientSession clientSession = client.startSession()) {
            return clientSession.withTransaction(() -> {
                people.forEach(p -> p.setId(new ObjectId()));
                personCollection.insertMany(clientSession, people);
                return people;
            }, txnOptions);
        }
    }
    ...
    @Override
    public long update(List<Person> people) {
        List<ReplaceOneModel<Person>> writes = people.stream()
