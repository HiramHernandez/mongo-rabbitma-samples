...
public class PersonServiceImpl implements  PersonService{
    ...
    public PersonDTO save(PersonDTO personDTO) {
        return new PersonDTO(personRepository.save(personDTO.toPersonModel()));
    }

    @Override
    public List<PersonDTO> saveAll(List<PersonDTO> people) {
        return people.stream()
                .map(PersonDTO::toPersonModel)
    ...
    public PersonDTO update(PersonDTO PersonDTO) {
        return new PersonDTO(personRepository.update(PersonDTO.toPersonModel()));
    }

    @Override
    public long update(List<PersonDTO> personEntities) {
        return personRepository.update(personEntities.stream().map(PersonDTO::toPersonModel).toList());
    }
